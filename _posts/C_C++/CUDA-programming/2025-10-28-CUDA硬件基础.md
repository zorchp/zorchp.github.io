...
- **多个 block 被分配到不同的 SM 上执行**  
- **每个 SM 内的 warp scheduler 调度 warp 级别的执行 ** 
- **每个 warp 内的 32 个线程同时执行相同指令（SIMT） ** 

| 软件抽象 | 硬件实体              | 特征               |
| -------- | --------------------- | ------------------ |
| `thread` | 执行流                | 在 warp 内并行执行 |
| `block`  | 调度与共享内存单位    | 在一个 SM 内执行   |
| `grid`   | kernel 启动的所有线程 | 分布在所有 SM 上   |
| `warp`   | 硬件调度单位          | 固定 32 线程       |
| `SM`     | GPU 的“核心”          | 执行多个 block     |
