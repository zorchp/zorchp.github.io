...
-   public: 能被所有用户 (类内成员和对象) 访问
-   protected: 能被 派生类成员 / 友元 访问
-   private: 能被 类内成员 / 友元 访问

这里最不好理解的就是 protected 级别, 因为这个级别既像 public(在派生类中)又像 private(在类的对象中). 
...
-   protected:
    ...
    3.   $\bigstar$ 派生类的成员或者友元**只能通过派生类对象**来访问基类的受保护成员, 派生类对于一个基类对象中的受保护成员没有任何访问权限
...
class Sneaky : public Base {
    friend void f(Sneaky&); // 可以访问 Sneaky::prot_mem
    friend void f(Base&);   // 不可访问 Base::prot_mem
...
// void f(Base& b) { b.prot_mem = 0; } // protected, 不可访问

```

至于为什么要有这样一条规则, 其原因是:

如果派生类及其友元能访问基类对象的受保护成员, 则上面参数为基类引用的函数 f 将是合法的, 即使这个函数不是基类的友元, 却可以访问(甚至改变) 基类的成员, 会造成访问级别提供的成员保护失效. 
