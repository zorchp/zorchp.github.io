...
public:
    Node* cloneGraph(Node* node) {
        ...
        queue<pair<Node*, Node*>> q; // need src and dest
        unordered_map<Node*, Node*> dict;
        auto ans = new Node(node->val);
        q.emplace(node, ans);
        dict[node] = ans;
        while (!q.empty()) {
            auto [x_src, x_dest] = q.front();
            q.pop();

            for (auto& y : x_src->neighbors) {
                if (!dict.count(y)) {
                    auto tmp = new Node(y->val);
                    q.emplace(y, tmp);
                    dict[y] = tmp;
                    x_dest->neighbors.push_back(tmp);
                } else {
                    x_dest->neighbors.push_back(dict[y]);
                }
            }
        }
        return ans;
