...
下面的这些数据结构都用到了链表, 可见链表在数据库缓存等底层构件中的重要作用. 
...
class LFUCache {
    struct Node {
        ...
        Node(int k, int v, int f, Node* p, Node* n)
            : key(k), val(v), freq(f), prev(p), next(n) {}
    };
    struct List {
        Node *head, *tail;
        ...
        Node* getTail() { return empty() ? nullptr : tail->prev; }
    };
    void inc(Node* node) {
        int freq = node->freq;
        auto list = freqMap[freq];
        list->delNode(node);
        if (freq == minFreq && list->empty()) {
            ++minFreq;
            freqMap.erase(freq);
            delete list->head;
            delete list->tail;
