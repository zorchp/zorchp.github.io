...
```cpp
float min_edit_dist(const std::vector<std::string>& patt1,
                    const std::vector<std::string>& patt2,
                    float w_add /* insert */ = 1.f,
                    float w_del /* delete */ = 1.f,
                    float w_sub /* substitute */ = 1.f)
{
    int m = patt1.size();
    int n = patt2.size();

    std::vector<std::vector<float>> dp(n + 1, std::vector<float>(m + 1));
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            ...
            if (patt1[j - 1] == patt2[i - 1]) {
                dp[i][j] = dp[i - 1][j - 1];  // match
            } else {
                float cost_add = dp[i - 1][j] + w_add;
                float cost_del = dp[i][j - 1] + w_del;
                float cost_sub = dp[i - 1][j - 1] + w_sub;

                dp[i][j] = std::min({cost_add, cost_del, cost_sub});
            }
        }
    }
