...
13.    [1048. 最长字符串链](https://leetcode.cn/problems/longest-string-chain/);
         ...
         int minCut(string s) {
             ...
             for (int r{}; r < n; ++r) {
                 if (g[0][r])
                     ...
                     for (int l{}; l < r; ++l)
                         if (g[l + 1][r]) dp[r] = min(dp[r], dp[l] + 1);
                 }
             }
             return dp[n - 1];
         }
...
7.    [2376. 统计特殊整数](https://leetcode.cn/problems/count-special-integers/);
     ```cpp
     ```

     
8.    [2719. 统计整数数目](https://leetcode.cn/problems/count-of-integers/);
     ```cpp
     ```
