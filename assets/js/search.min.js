var elSearchBox = document.querySelector('.search'),
    ...
    searchValue = '', arrItems = [], arrContents = [], arrLinks = [],
    arrTitles = [], arrResults = [], indexItem = [], itemLength = 0,
    tmpDiv = document.createElement('div'),
    tmpAnchor = document.createElement('a'), isSearchFocused = !1,
...
function searchConfirm() {
    if ('' == elSearchInput.value)
        ...
        e.innerText = '请输入有效内容...', elSearchResults.appendChild(e)
    } else
        searchInit(), searchValue = elSearchInput.value,
                      searchMatching(arrTitles, arrContents, searchValue)
}
function searchClear() {
    elSearchInput.value = '', elSearchClear.style.display = 'none',
    elSearchResults.style.display = 'none',
    elSearchResults.classList.remove('result-item')
}
function searchInit() {
    arrResults = [], indexItem = [], elSearchResults.innerHTML = '',
...
function searchMatching(e, t, r) {
    ...
    for (i = 0; i < itemLength; i++) {
        ...
        if (-1 !== c || -1 !== h) {
            ...
            arrResults.push(n[i].slice(s - 10, s) + '<mark>' +
                            n[i].slice(s, s + o) + '</mark>' +
                            n[i].slice(s + o, s + o + 10))
        }
    }
    var u = tmpDiv.cloneNode(!0);
    if (u.className = 'result-title',
        u.innerHTML = '总匹配：<b>' + indexItem.length + '</b> 项',
        elSearchResults.appendChild(u), 0 == indexItem.length) {
    ...
    for (i = 0; i < arrResults.length; i++) {
        var m = tmpDiv.cloneNode(!0), p = tmpDiv.cloneNode(!0),
            S = tmpDiv.cloneNode(!0), v = tmpAnchor.cloneNode(!0);
        m.className = 'card hoverable result-item',
        p.className = 'card-content result-item-title',
        S.className = 'card-action result-item-detail',
        v.className = 'blue-text', p.innerText = e[indexItem[i]],
        v.innerHTML = arrResults[i], v.href = arrLinks[indexItem[i]],
        m.appendChild(p), S.appendChild(v), m.appendChild(S),
...
xhr.onreadystatechange =
    ...
    if (4 == xhr.readyState && 200 == xhr.status) {
        ...
        for (arrItems = e.getElementsByTagName('item'),
            itemLength = arrItems.length, i = 0;
             i < itemLength; i++)
    ...
    xhr.open('get', '/feed.xml', !0), xhr.send(),
    elSearchBtn.onclick = searchConfirm, elSearchClear.onclick = searchClear,
